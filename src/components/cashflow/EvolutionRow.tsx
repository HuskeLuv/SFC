...
export const EvolutionRow: React.FC<EvolutionRowProps> = ({
  ...
  <TableRow className="h-6 bg-[#998256] text-white w-full" style={{ fontFamily: "Calibri, sans-serif", fontSize: "12px" }}>
    ...
    {valuesByMonth.map((value, index) => {
      ...
      const colorClass = numValue >= 0 ? "text-white" : "text-red-600 dark:text-red-400";
      const borderClass = index === 0 ? "border-l-0" : "border-l border-gray-200";
      return (
        <TableCell
          key={index}
          className={`px-1 font-bold border-t border-b border-gray-200 border-r border-gray-200 text-xs text-right h-6 leading-6 ${borderClass} ${colorClass}`}
          style={{ minWidth: "3rem" }}
        >
    ...
    <TableCell
      className={`px-2 font-bold border-t border-b border-gray-200 border border-gray-200 text-xs text-right h-6 leading-6 ${
        totalAnnual >= 0 ? "text-white" : "text-red-600 dark:text-red-400"
      }`}
      style={{ minWidth: "4rem" }}
    >
