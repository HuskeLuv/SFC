...
export const useFii = () => {
  ...
  const calculateAtivoValues = (
    ...
    const riscoPorAtivo =
      totalCarteiraFii > 0 ? Math.min(100, (valorAtualizado / totalCarteiraFii) * 100) : 0;
    // Percentual daquele tipo de ativo (não da carteira total)
    const percentualCarteira =
      totalCarteiraFii > 0 ? (valorAtualizado / totalCarteiraFii) * 100 : 0;
    ...
    const necessidadeAporte =
      totalCarteiraFii > 0 && quantoFalta > 0 ? (quantoFalta / 100) * totalCarteiraFii : 0;
    const rentabilidade =
  ...
  const updateCaixaParaInvestir = useCallback(
    async (novoCaixa: number) => {
      try {
        const response = await csrfFetch('/api/carteira/fii', {
          method: 'POST',
          headers: {
  ...
  const updateObjetivo = async (ativoId: string, novoObjetivo: number) => {
    ...
    try {
      ...
      // Fazer chamada à API
      const response = await csrfFetch('/api/carteira/fii/objetivo', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const updateCotacao = async (ativoId: string, novaCotacao: number) => {
    try {
      const response = await csrfFetch('/api/carteira/fii/cotacao', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
