...
export const useEtf = () => {
  ...
  const updateCaixaParaInvestir = useCallback(
    async (novoCaixa: number) => {
      try {
        const response = await csrfFetch('/api/carteira/etf', {
          method: 'POST',
          headers: {
  ...
  const updateObjetivo = async (ativoId: string, novoObjetivo: number) => {
    ...
    try {
      ...
      // Fazer chamada à API
      const response = await csrfFetch('/api/carteira/etf/objetivo', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const updateCotacao = async (ativoId: string, novaCotacao: number) => {
    try {
      const response = await csrfFetch('/api/carteira/etf/cotacao', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
