...
const normalizeConsultantId = async (consultantId: string) => {
  ...
  const consultant = await prisma.consultant.findFirst({
    where: {
      OR: [{ id: consultantId }, { userId: consultantId }],
    },
    include: {
      user: true,
      clients: {
...
export const getClientsByConsultant = async (
  consultantId: string,
): Promise<ConsultantClientDescriptor[]> => {
...
export const getClientSummary = async (clientId: string): Promise<ClientSummary | null> => {
  ...
  const monthlyReturnPercentageRaw =
    portfolioSnapshot.totalInvested > 0
...
export const getConsultantOverview = async (
  consultantId: string,
): Promise<ConsultantOverview | null> => {
  const consultant = await normalizeConsultantId(consultantId);
  if (!consultant) {
    return null;
  }
  ...
  const validSummaries = summaries.filter(
    (entry): entry is { summary: ClientSummary; status: ConsultantClientStatus } =>
  ...
  if (validSummaries.length === 0) {
    return {
      ...
      totalActiveClients: clients.filter(({ status }) => status === ConsultantClientStatus.active)
        .length,
      totalManagedAssets: 0,
      averageClientReturn: 0,
    };
  ...
  const averageClientReturnRaw =
    validSummaries.reduce((total, { summary }) => total + summary.monthlyReturnPercentage, 0) /
...
const getClientAportesResgates = async (
  clientId: string,
  year?: number,
): Promise<{ aportes: number; resgates: number }> => {
  const targetYear = year || new Date().getFullYear();
  const yearStart = new Date(targetYear, 0, 1);
  const yearEnd = new Date(targetYear, 11, 31, 23, 59, 59);
...
export const getTopClientsByReturn = async (
  consultantId: string,
  limit = 5,
  ...
  return clientReturns.sort((a, b) => b.totalReturn - a.totalReturn).slice(0, limit);
};

export const getTopClientsByPatrimony = async (
  consultantId: string,
  limit = 5,
): Promise<ClientPatrimony[]> => {
  const consultant = await normalizeConsultantId(consultantId);
  if (!consultant) {
    return [];
  }
  ...
  return clientPatrimonies.sort((a, b) => b.patrimony - a.patrimony).slice(0, limit);
};

export const getClientWithHighestPatrimony = async (
  consultantId: string,
): Promise<ClientPatrimony | null> => {
...
export const getTopClientsBySavingRate = async (
  consultantId: string,
  limit = 5,
): Promise<ClientSavingRate[]> => {
  const consultant = await normalizeConsultantId(consultantId);
  if (!consultant) {
    return [];
  }
...
export const getClientsWithoutAportes = async (
  consultantId: string,
  daysThreshold = 90,
): Promise<RiskAlert[]> => {
  const consultant = await normalizeConsultantId(consultantId);
...
export const getClientsHighConcentration = async (
  consultantId: string,
  threshold = 60,
): Promise<RiskAlert[]> => {
  const consultant = await normalizeConsultantId(consultantId);
...
export const getAportesResgatesByClient = async (
  consultantId: string,
  year?: number,
): Promise<AportesResgates[]> => {
  const consultant = await normalizeConsultantId(consultantId);
  if (!consultant) {
    return [];
  }
...
export const getPatrimonyEvolution = async (
  consultantId: string,
  months = 12,
): Promise<PatrimonyEvolution[]> => {
  const consultant = await normalizeConsultantId(consultantId);
  if (!consultant) {
    return [];
  }
