...
export const GET = withErrorHandler(async (request: NextRequest) => {
  ...
  const tesouroReservaEmergenciaAssetIds = new Set<string>();
  for (const tx of userPurchaseNotes) {
    if (!tx.assetId || !tx.notes) continue;
    try {
      const parsed = JSON.parse(tx.notes);
      if (parsed?.tesouroDestino === 'reserva-emergencia') {
        tesouroReservaEmergenciaAssetIds.add(tx.assetId);
      }
    } catch {
  ...
  const portfolio = allUserPortfolio.filter((item) => {
    ...
    if (item.asset.type === 'emergency') return true;
    if (item.asset.symbol?.startsWith('RESERVA-EMERG')) return true;
  ...
  const ativos = portfolio.map((item) => {
    ...
    // Rentabilidade descontando fluxos (report 14/08/2026 — fórmula do
    // Wellington): atual / (inicial + aportes − resgates) − 1. Aporte não é
    // rendimento e resgate reduz a base no custo — a fórmula antiga
    // ((atual − inicial) / inicial) mostrava −32% num CDB que só teve resgate.
  ...
  // base = inicial + aportes − resgates; rendimento = atual − base. Fonte
  // única do card e da linha TOTAL GERAL.
