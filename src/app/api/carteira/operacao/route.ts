...
export async function POST(request: NextRequest) {
  try {
    ...
    if (tipoAtivo === 'conta-corrente') {
      ...
      if (!dataInicio || !valorAplicado || !contaCorrenteDestino) {
        return NextResponse.json(
          {
            error:
              'Campos obrigatórios para conta corrente: dataInicio, valorAplicado, contaCorrenteDestino',
          },
          { status: 400 },
        );
      }
      ...
      if (!dataInicio || !valorAplicado || !contaCorrenteDestino) {
        return NextResponse.json(
          {
            error:
              'Campos obrigatórios para poupança: dataInicio, valorAplicado, contaCorrenteDestino (onde exibir)',
          },
          { status: 400 },
        );
      }
      ...
      if (
        ...
        return NextResponse.json(
          {
            error:
              'Campos obrigatórios para este tipo: dataInicio, nomePersonalizado, quantidade, precoUnitario, metodo',
          },
          { status: 400 },
        );
      }
      ...
      if (!dataInicio || !emissorId || !periodo || !valorAplicado || !taxaJurosAnual) {
        return NextResponse.json(
          {
            error:
              'Campos obrigatórios para este tipo: dataInicio, emissorId, periodo, valorAplicado, taxaJurosAnual',
          },
          { status: 400 },
        );
      }
      ...
      if (!rendaFixaTipo || !dataInicio || !dataVencimento || !valorAplicado || !descricao) {
        return NextResponse.json(
          {
            error:
              'Campos obrigatórios para este tipo: rendaFixaTipo, dataInicio, dataVencimento, valorAplicado, descricao',
          },
          { status: 400 },
        );
      }
      ...
      if (
        tipoAtivo === 'renda-fixa-posfixada' &&
        (!rendaFixaIndexer || !['CDI', 'IPCA'].includes(rendaFixaIndexer))
      ) {
        return NextResponse.json(
          {
      ...
      if (
        ...
        return NextResponse.json(
          {
            error:
              'tesouroDestino é obrigatório. Selecione onde o título deve aparecer: Reserva de Emergência, Reserva de Oportunidade ou Renda Fixa (Pré/Pós/Híbrida).',
          },
          { status: 400 },
        );
      }
      ...
      if (tesouroEmReserva) {
        if (
          ...
          return NextResponse.json(
            {
              error:
                'Campos obrigatórios para tesouro em reserva: dataCompra, valorInvestido, cotizacaoResgate, liquidacaoResgate, vencimento, benchmark',
            },
            { status: 400 },
          );
        }
      ...
      if (tesouroEmRendaFixa) {
        ...
        if (metodoCotasTesouro) {
          if (!quantidade || !cotacaoUnitaria || quantidade <= 0 || cotacaoUnitaria <= 0) {
            return NextResponse.json(
              { error: 'Para adição por cotas: quantidade e preço da cota são obrigatórios' },
              { status: 400 },
            );
          }
        } else {
          if (!valorInvestido || valorInvestido <= 0) {
            return NextResponse.json(
              { error: 'Valor investido é obrigatório e deve ser maior que zero' },
              { status: 400 },
            );
          }
        }
        if (!dataVencimento || !requestBody.descricao) {
          return NextResponse.json(
            { error: 'Data de vencimento e descrição são obrigatórios' },
            { status: 400 },
          );
        }
        if (dest === 'renda-fixa-prefixada' && (!taxaJurosAnual || taxaJurosAnual <= 0)) {
          return NextResponse.json(
            { error: 'Taxa de juros anual é obrigatória para tesouro pré-fixado' },
            { status: 400 },
          );
        }
        if (
          (dest === 'renda-fixa-posfixada' || dest === 'renda-fixa-hibrida') &&
          (!rendaFixaIndexer || !['CDI', 'IPCA'].includes(rendaFixaIndexer))
        ) {
          return NextResponse.json(
            { error: 'Indexador (CDI ou IPCA) é obrigatório para tesouro pós-fixado/híbrido' },
            { status: 400 },
          );
        }
      }
      ...
      if (
        tipoAtivo === 'debenture' &&
        (!tipoDebenture || !['prefixada', 'pos-fixada', 'hibrida'].includes(tipoDebenture))
      ) {
        return NextResponse.json(
          {
      ...
      if (
        ...
        (!fundoRendaFixaTipo ||
          !['prefixada', 'pos-fixada', 'hibrida'].includes(fundoRendaFixaTipo))
      ) {
        return NextResponse.json(
          {
            error:
              'Tipo de renda fixa é obrigatório para fundo em Renda Fixa. Selecione Pré-fixada, Pós-fixada ou Híbrida.',
          },
          { status: 400 },
        );
      }
      if (metodoCotas) {
        if (!quantidade || !cotacaoUnitaria || quantidade <= 0 || cotacaoUnitaria <= 0) {
          return NextResponse.json(
            {
              error:
                'Para adição por cotas: quantidade e preço da cota são obrigatórios e devem ser maiores que zero',
            },
            { status: 400 },
          );
        }
      ...
      if (
        ...
        return NextResponse.json(
          {
            error:
              'Campos obrigatórios para stocks: dataCompra, quantidade, cotacaoUnitaria, moeda, cotacaoMoeda, estrategia',
          },
          { status: 400 },
        );
      }
      ...
      if (tipoAtivo === 'opportunity' && !(ativoNome || '').trim()) {
        return NextResponse.json(
          {
            error:
              'Nome do ativo é obrigatório para reserva de oportunidade. Informe o nome que será exibido na tabela.',
          },
          { status: 400 },
        );
      }
      if (tipoAtivo === 'emergency' && !(ativoNome || '').trim()) {
        return NextResponse.json(
          {
            error:
              'Nome do ativo é obrigatório para reserva de emergência. Informe o nome que será exibido na tabela.',
          },
          { status: 400 },
        );
      }
      ...
      if (!dataCompra || !quantidade || !cotacaoUnitaria || !estrategia) {
        return NextResponse.json(
          {
            error:
              'Campos obrigatórios para ações: dataCompra, quantidade, cotacaoUnitaria, estrategia',
          },
          { status: 400 },
        );
      }
      ...
      if (!dataCompra || !quantidade || !cotacaoUnitaria || !cotacaoMoeda || !estrategiaReit) {
        return NextResponse.json(
          {
            error:
              'Campos obrigatórios para REIT: dataCompra, quantidade, cotacaoUnitaria (USD), cotacaoMoeda (cotação do dólar no câmbio), estrategiaReit',
          },
          { status: 400 },
        );
      }
      ...
      if (!dataCompra || !quantidade || !cotacaoUnitaria || !regiaoEtf) {
        return NextResponse.json(
          {
            error:
              'Campos obrigatórios para ETF: dataCompra, quantidade, cotacaoUnitaria, regiaoEtf (região: brasil ou estados_unidos)',
          },
          { status: 400 },
        );
      }
      ...
      if (
        ...
        return NextResponse.json(
          {
            error:
              'Campos obrigatórios para opções: dataCompra, quantidade, cotacaoUnitaria, opcaoTipo (put/call), opcaoCompraVenda (compra/venda), dataVencimento',
          },
          { status: 400 },
        );
      }
      ...
      if (!tickerAtivo) {
        return NextResponse.json(
          { error: 'Ticker do ativo base é obrigatório para opções' },
          { status: 400 },
        );
      }
    } else {
    ...
    if (
      (tipoAtivo === 'acao' || tipoAtivo === 'bdr' || tipoAtivo === 'fii') &&
      (quantidade <= 0 || cotacaoUnitaria <= 0)
    ) {
      return NextResponse.json(
        {
    ...
    if (
      (tipoAtivo === 'conta-corrente' ||
        tipoAtivo === 'poupanca' ||
        tipoAtivo === 'renda-fixa-prefixada' ||
        tipoAtivo === 'renda-fixa-posfixada' ||
        tipoAtivo === 'renda-fixa') &&
      valorAplicado <= 0
    ) {
    ...
    if (
      (tipoAtivo === 'renda-fixa' || tipoAtivo === 'renda-fixa-posfixada') &&
      taxaJurosAnual <= 0
    ) {
      return NextResponse.json(
        {
    ...
    if (
      (tipoAtivo === 'renda-fixa' || tipoAtivo === 'renda-fixa-posfixada') &&
      rendaFixaIndexerPercent !== undefined &&
      rendaFixaIndexerPercent !== null
    ) {
    ...
    const debentureFundoPrevidenciaMetodoCotas =
      (tipoAtivo === 'debenture' ||
        tipoAtivo === 'tesouro-direto' ||
        tipoAtivo === 'fundo' ||
        tipoAtivo === 'previdencia') &&
      (requestBody.metodo === 'cotas' || requestBody.metodo === 'percentual');
    const valorInvestidoOuCalculado =
      tipoAtivo === 'moeda'
    ...
    if (tipoAtivo === 'conta-corrente') {
      ...
      const baseName =
        contaCorrenteDestino === 'reserva-emergencia'
          ? 'Conta Corrente (Reserva Emergência)'
          : 'Conta Corrente (Reserva Oportunidade)';
      const baseSymbol =
        contaCorrenteDestino === 'reserva-emergencia'
          ? 'CONTA-CORRENTE-EMERG'
          : 'CONTA-CORRENTE-OPORT';
      ...
      const valorFormatado = valorAplicado
      ...
      const baseName =
        contaCorrenteDestino === 'reserva-emergencia'
          ? 'Poupança (Reserva Emergência)'
          : 'Poupança (Reserva Oportunidade)';
      const baseSymbol =
        contaCorrenteDestino === 'reserva-emergencia' ? 'POUPANCA-EMERG' : 'POUPANCA-OPORT';
      const timestamp = Date.now();
      const uniqueId = Math.random().toString(36).substring(2, 9);
      const assetSymbol = `${baseSymbol}-${timestamp}-${uniqueId}`;
      const dataFormatada = new Date(dataInicio || new Date()).toLocaleDateString('pt-BR');
      ...
      const dataFormatada = new Date(dataCompra || dataInicio || new Date()).toLocaleDateString(
        'pt-BR',
      );
      ...
      if (tipoAtivo === 'personalizado') {
        ...
        const valorFormatado = valorInvestido
          ? new Intl.NumberFormat('pt-BR', {
              style: 'currency',
              currency: 'BRL',
              minimumFractionDigits: 0,
      ...
      const metodoCotasFundo =
        requestBody.metodo === 'cotas' || requestBody.metodo === 'percentual';
      const valorParaNome =
        metodoCotasFundo && quantidade > 0 && cotacaoUnitaria > 0
          ? quantidade * cotacaoUnitaria
          : valorInvestido;
      const timestamp = Date.now();
      const uniqueId = Math.random().toString(36).substring(2, 9);
      ...
      const valorParaNome =
        metodoCotasDeb && quantidade > 0 && cotacaoUnitaria > 0
          ? quantidade * cotacaoUnitaria
          : valorInvestido;
      const timestamp = Date.now();
      const uniqueId = Math.random().toString(36).substring(2, 9);
      ...
      if (!nomeReit) {
        return NextResponse.json(
          { error: 'Ticker ou nome do REIT é obrigatório' },
          { status: 400 },
        );
      }
      const valorTotalReit =
        quantidade > 0 && cotacaoUnitaria > 0 ? quantidade * cotacaoUnitaria : valorInvestido;
      const timestamp = Date.now();
      const uniqueId = Math.random().toString(36).substring(2, 9);
      const safeSymbol =
        nomeReit
      ...
      const dataFormatada = new Date(dataCompra || new Date()).toLocaleDateString('pt-BR');
      const valorFormatado = valorTotalReit
        ? new Intl.NumberFormat('en-US', {
            style: 'currency',
            currency: 'USD',
            minimumFractionDigits: 0,
            maximumFractionDigits: 0,
          }).format(valorTotalReit)
        : '';
      ...
      const valorTotalStock =
        quantidade > 0 && cotacaoUnitaria > 0 && cotacaoMoeda > 0
      ...
      const assetSymbol = `${safeSymbol}-${timestamp}-${uniqueId}`;
      const dataFormatada = new Date(dataCompra || new Date()).toLocaleDateString('pt-BR');
      const valorFormatado = valorTotalStock
        ? new Intl.NumberFormat('pt-BR', {
            style: 'currency',
            currency: 'BRL',
            minimumFractionDigits: 0,
            maximumFractionDigits: 0,
          }).format(valorTotalStock)
        : '';
      ...
      const _valorTotalOpcao = quantidade * cotacaoUnitaria + (taxaCorretagem || 0);
      const timestamp = Date.now();
      const uniqueId = Math.random().toString(36).substring(2, 9);
      ...
      if (!nomeTesouro) {
        return NextResponse.json(
          { error: 'Nome do título do Tesouro Direto é obrigatório' },
          { status: 400 },
        );
      }
      const dest = tesouroDestino;
      const tesouroEmReserva = dest === 'reserva-emergencia' || dest === 'reserva-oportunidade';
      const _tesouroEmRendaFixa =
        dest === 'renda-fixa-prefixada' ||
        dest === 'renda-fixa-posfixada' ||
        dest === 'renda-fixa-hibrida';

      const valorParaNome = tesouroEmReserva
        ? valorInvestido
        : (requestBody.metodo === 'cotas' || requestBody.metodo === 'percentual') &&
            quantidade > 0 &&
            cotacaoUnitaria > 0
      ...
      if (!assetId) {
        return NextResponse.json(
          { error: 'assetId é obrigatório para ações e FIIs' },
          { status: 400 },
        );
      }
