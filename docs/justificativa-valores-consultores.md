...
**Reprocesso após edição (Bug #02 — Maio/2026):** ao editar ou apagar uma transação histórica, o sistema invalida automaticamente os snapshots diários (`portfolio_daily_snapshots`) e a série TWR pré-calculada (`portfolio_performance`) **a partir do dia da edição**. O cache em memória do `/carteira/resumo` do usuário também é limpo. Sem essa invalidação, a série continuaria contabilizando o cashflow antigo: ao editar a data de um aporte, o algoritmo lê o saldo no "novo dia" sem reconhecer o aporte → trata a diferença como valorização → barras mensais explodem (caso real: 163% num mês). O reader detecta a quebra de cobertura (`coverageOk=false`) e cai automaticamente no builder ao vivo até o cron diário repovoar o histórico. Para o consultor, isso significa que **após qualquer correção retroativa nas movimentações do cliente**, o próximo carregamento de Análises mostra a série já recalculada — não é mais preciso esperar o cron rodar. Um warning é emitido em log quando o retorno diário ultrapassa 5% (sentinela contra regressões).

**Sincronia de datas em Renda Fixa (Bug #04 — Maio/2026):** ao editar a data da movimentação inicial de um CDB/LCI/LCA, o `recalculatePortfolioFromTransactions` também atualiza `FixedIncomeAsset.startDate` e regenera o `Asset.name` (template "{prefixo} - R$ X - dd/mm/aaaa"). Sem isso, o cabeçalho da tela de detalhes ficava preso na data antiga, mesmo após o usuário confirmar a edição. Nomes customizados (fora do padrão do template) são preservados intactos. A nova `startDate` também passa a alimentar a marcação na curva (CDI/PRE/IPCA) — o que altera o valor exibido conforme o número de dias úteis acumulados.

**YTD zerado (Bug #03 — Maio/2026):** o sintoma — Carteira "No ano" exibindo 0,00% enquanto CDI/IBOV mostram valores positivos — era cascata direta do bug #02 (série diária contaminada por cashflows obsoletos zerava o TWR cumulativo). O fix de #02 resolve a fonte. Como sentinela contra regressões, o endpoint `/api/analises/rentabilidade-janelas` agora **emite warning em log** quando a janela YTD retorna 0 mas o CDI no mesmo período é positivo (sinal claro de série quebrada). Se isso aparecer em produção depois do fix, o consultor deve reportar para investigação.
